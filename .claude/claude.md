...
| 16 | 7 color | 0 | ⚠️ UNVERIFIED wheel positions |
| 17 | Reset | 0 | |
...
|----|----------|-------------|-------|
| 0 | Dimmer | 255 | |
| 1 | **Strobe** | **0** | ⚠️ UNVERIFIED — uses ShutterStrobeSlowFast preset, exact ranges TBD |
| 2 | Pan | 0 (center) | |
| 3 | Tilt | 123 (center) | |
| 4 | PT Speed | 0 | |
| 5 | Color | 0 | ⚠️ UNVERIFIED wheel positions |
...
- **shows/** — All .qxw workspace files live here.
...
- **songs/** — Audio files for analysis. Drop tracks here before running allin1.
- **songs-data/** — allin1 output (JSON files with BPM, beats, downbeats, segments). Generated by `analyze.sh`.
- **Dockerfile** — Builds the allin1 music analysis Docker image (CPU-only, PyTorch 2.6.0 + NATTEN 0.17.5).
- **analyze.sh** — Runs allin1 on all files in `songs/`, outputs to `songs-data/`. Usage: `./analyze.sh` or `./analyze.sh "filename.flac"`.
