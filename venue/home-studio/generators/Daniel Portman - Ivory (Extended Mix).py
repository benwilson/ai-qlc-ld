#!/usr/bin/env python3
"""
Show Generator: Daniel Portman - Ivory (Extended Mix)
=====================================================
BPM: 125 | Duration: ~7:44 | Genre: Melodic Techno

Design intent:
- Mover position changes quantized to 4 beats (1 bar).
- PAR behavior uses fade/blink modes mapped to 1/2/4/8-beat intent.
- Section-aware energy mapping from songs-data analysis JSON.
- Strong variety via pattern rotation + palette rotation.
"""
...
SONG_STEM = "Daniel Portman - Ivory (Extended Mix)"
...
PALETTES = {
    "deep_indigo": {
        "par_a": (10, 14, 70), "par_b": (30, 10, 90),
        "miss1": (18, 18, 80), "miss2": (12, 10, 40),
        "sh": SHARPY_BLUE, "bsw": BSW_BLUE, "prof": PROF_BLUE,
        "halo": H_BLU, "ni": (20, 30, 100)
    },
    "teal_steel": {
        "par_a": (0, 90, 110), "par_b": (20, 40, 80),
        "miss1": (0, 110, 125), "miss2": (0, 60, 80),
        "sh": SHARPY_TEAL, "bsw": BSW_TEAL, "prof": PROF_TEAL,
        "halo": H_CYN, "ni": (0, 120, 140)
    },
    "violet_gold": {
        "par_a": (110, 20, 165), "par_b": (225, 160, 40),
        "miss1": (150, 40, 180), "miss2": (180, 120, 30),
        "sh": SHARPY_PURPLE, "bsw": BSW_MAG, "prof": PROF_PINK,
        "halo": H_PNK, "ni": (130, 35, 170)
    },
    "amber_cobalt": {
        "par_a": (200, 95, 15), "par_b": (20, 40, 125),
        "miss1": (220, 120, 20), "miss2": (20, 65, 170),
        "sh": SHARPY_AMBER, "bsw": BSW_ORANGE, "prof": PROF_ORANGE,
        "halo": H_YEL, "ni": (160, 70, 25)
    },
    "magenta_cyan": {
        "par_a": (190, 10, 120), "par_b": (0, 190, 210),
        "miss1": (210, 30, 150), "miss2": (0, 190, 225),
        "sh": SHARPY_PINK, "bsw": BSW_MAG, "prof": PROF_PINK,
        "halo": H_PNK, "ni": (200, 20, 140)
    },
    "ruby_ice": {
        "par_a": (230, 20, 30), "par_b": (130, 190, 255),
        "miss1": (255, 25, 35), "miss2": (130, 180, 255),
        "sh": SHARPY_RED, "bsw": BSW_RED, "prof": PROF_RED,
        "halo": H_RED, "ni": (220, 25, 35)
    },
    "ivory_white": {
        "par_a": (255, 220, 165), "par_b": (175, 205, 255),
        "miss1": (255, 220, 180), "miss2": (175, 205, 255),
        "sh": SHARPY_WHITE, "bsw": BSW_WHITE, "prof": PROF_WHITE,
        "halo": H_RGB, "ni": (255, 230, 185)
    },
    "acid_lime": {
        "par_a": (130, 225, 0), "par_b": (40, 100, 0),
        "miss1": (150, 240, 20), "miss2": (70, 140, 0),
        "sh": SHARPY_LIME, "bsw": BSW_GREEN, "prof": PROF_GREEN,
        "halo": H_GRN, "ni": (145, 230, 20)
    },
    "night_void": {
    ...
    "warm_ash": {
        "par_a": (120, 70, 25), "par_b": (80, 45, 20),
        "miss1": (150, 95, 35), "miss2": (90, 50, 20),
        "sh": SHARPY_ORANGE, "bsw": BSW_ORANGE, "prof": PROF_ORANGE,
        "halo": H_YEL, "ni": (115, 70, 30)
    },
}
...
def pick_palette(phrase: str, seg_idx: int, rms: float, seg_bar_idx: int) -> str:
    if phrase == "intro":
        seq = ["deep_indigo", "teal_steel", "violet_gold"]
        return seq[(seg_idx + seg_bar_idx // 4) % len(seq)]
    if phrase == "breakdown":
        seq = ["night_void", "deep_indigo", "violet_gold"]
        return seq[(seg_idx + seg_bar_idx // 4) % len(seq)]
    if phrase == "outro":
        seq = ["warm_ash", "ivory_white", "deep_indigo"]
        return seq[(seg_idx + seg_bar_idx // 4) % len(seq)]
    if phrase == "drop":
        if seg_bar_idx % 8 == 0:
            return "ivory_white"
        seq = ["magenta_cyan", "ruby_ice", "amber_cobalt", "acid_lime"]
        return seq[(seg_idx + seg_bar_idx // 2) % len(seq)]
    # verse_groove, build, and fallback
    seq = ["teal_steel", "amber_cobalt", "violet_gold", "deep_indigo"]
    if rms > 0.70:
        seq = ["amber_cobalt", "magenta_cyan", "teal_steel", "ruby_ice"]
...
scenes = [scene("Blackout", *blackout_all(), path="Ivory")]
